...
export default function SuspendUserModal({
    ...
    onClose,
    onConfirmStatus,
}: SuspendUserModalProps): React.JSX.Element | null {
    ...
    return (
        <Modal
            ...
            icon={
                isCurrentlyBlocked ? (
                    <UserCheck size={20} className="text-[#00e5c0]" />
                ) : (
                    <UserX size={20} className="text-[#f43f5e]" />
                )
            }
            title={isCurrentlyBlocked ? "Reactivate User Account" : "Suspend / Block User"}
